...
from common.stripe_service import create_checkout_session
...
def handle_billing_checkout_session(event, body=None):
    if body is None:
        body = {}
    ...
    if not team_id or tier not in ("plus", "pro"):
    ...
    try:
        session = create_checkout_session(team, tier)
        return ok({"url": session.get("url")})
    except ValueError as e:
        return err(str(e), 400, code="validation_error")
    except Exception as e:
        print(f"[billing_checkout_session] Error: {e}")
        return err("Failed to create checkout session.", 500, code="server_error")
